...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
    $stmt = $db->prepare("INSERT INTO products (name, sku, category_id, price, stock_quantity, low_stock_threshold, barcode, description, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->execute([
        $_POST['name'], $_POST['sku'], $_POST['category_id'], $_POST['price'], $_POST['stock_quantity'], $_POST['low_stock_threshold'],
        $_POST['barcode'], $_POST['description'], $_POST['status']
    ]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    $stmt = $db->prepare("UPDATE products SET name=?, sku=?, category_id=?, price=?, stock_quantity=?, low_stock_threshold=?, barcode=?, description=?, status=? WHERE id=?");
    $stmt->execute([
        $_POST['name'], $_POST['sku'], $_POST['category_id'], $_POST['price'], $_POST['stock_quantity'], $_POST['low_stock_threshold'],
        $_POST['barcode'], $_POST['description'], $_POST['status'], $_POST['id']
    ]);
    header('Location: manage_product.php');
    exit();
}
...
<table class="table table-bordered table-sm">
    <thead>
        <tr>
            <th>Name</th><th>SKU</th><th>Category</th><th>Price</th><th>Stock Qty</th><th>Low Stock Threshold</th><th>Barcode</th><th>Description</th><th>Status</th><th>Actions</th>
        </tr>
    </thead>
    <tbody>
